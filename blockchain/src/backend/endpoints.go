...
var blockChainInfo *blockchain.BlockChainInfo = &blockchain.BlockChainInfo{
...
func CreateNewFile(w http.ResponseWriter, r *http.Request) {
	...
	json.NewEncoder(w).Encode(AddressResponse{address})
	return
}
...
func GetFileName(w http.ResponseWriter, r *http.Request) {
	...
	fa := &blockchain.FileAddress{faddress}
...
func UploadFile(w http.ResponseWriter, r *http.Request) {
	...
	faddress := r.Header.Get("FileAddress")
	fa := &blockchain.FileAddress{faddress}
	bc, err := makeConnection(ftext.PrivateKey)
	if err != nil {
		log.Println(err)
		json.NewEncoder(w).Encode(Message{err.Error()})
		return
